...
public class Torus extends Elips{
    ...
    int ibo;
    public Torus(List<ShaderModuleData> shaderModuleDataList, List<Vector3f> vertices, Vector4f color, float centerx, float centery, float centerz, float radiusx, float radiusy, float radiusz, Camera camera, Projection projection) {
        super(shaderModuleDataList, vertices, color, centerx, centery, radiusx, radiusy);
        this.centerz = centerz;
        this.radiusz = radiusz;
        this.camera = camera;
        this.projection = projection;
        createTorus(radiusx, radiusy, 36,36);
        setupVAOVBO();
    }
